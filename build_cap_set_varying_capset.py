...
def get_rid_of_capset_method(n : int,capset_size_min: int,capset_size_max : int, randomchance : int =0) -> np.ndarray[np.ndarray[int]]:
    """
    ...
    capset_size = randint(capset_size_min,capset_size_max)
...
def run(params : list):
    ...
    """
    n,sample_number, file,random_chance,capset_size_min,capset_size_max = params
    with open(file, "w") as f:
        f.write(f"n: {n} capset_size: {capset_size_min}-{capset_size_max} random_chance: {random_chance}\n")
    capsets = []
    for i in range(sample_number):
        with open(file, "a") as f:
            capset = get_rid_of_capset_method(n,capset_size_min=capset_size_min,capset_size_max=capset_size_max,randomchance=random_chance)
...
def main():
    ...
    for i in range(11):
        params.append([n,sample_size, f"logs{i}.txt", i*10,   capset_size_min,capset_size_max])
